...
const Output = struct { plane0: []u8, plane1: []u8 = undefined };
...
pub fn EncodePixelsToF32(fmt: TinyImageFormat, input: []@Vector(4, f32), output: Output) void {
    switch (fmt) {
        .R1_UNORM => {
            ...
            const dest = @ptrCast([*] format, @alignCast(@alignOf(format), output.plane0))[0..(output.plane0.len/8)];
        ...
        .R2_UNORM => {
            ...
            const dest = @ptrCast([*] format, @alignCast(@alignOf(format), output.plane0))[0..(output.plane0.len/4)];
            std.debug.assert(dest.len != 0);
            var si : usize = 0;
            var di : usize = 0;
        ...
        .R4_UNORM => {
            ...
            const dest = @ptrCast([*] format, @alignCast(@alignOf(format), output.plane0))[0..(output.plane0.len/2)];
            std.debug.assert(dest.len != 0);
            var si : usize = 0;
            var di : usize = 0;
        ...
        .R4G4_UNORM => {
            ...
            const dest = @ptrCast([*] format, @alignCast(@alignOf(format), output.plane0))[0..(output.plane0.len/1)];
            std.debug.assert(dest.len != 0);
            var si : usize = 0;
            var di : usize = 0;
